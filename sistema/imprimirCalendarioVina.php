...
<table border="1">
  <tr>
  	<td colspan="6" style="text-align:center">DermoSalud  <?php echo fechaEnLetras($ano."-".$mes."-".$dia);?></td>
    
  </tr>
    <tr>
  	<td colspan="6" style="text-align:center">Viña del Mar</td>
  </tr>
  <tr>
    <th>Hora</th>
    <th>Nombres</th>
  ...
  $citasTotal = Cita::listarRangoFecha($fechaInicio, $fechaFin, 1); 
  ...
  for($i=0;$i<73;$i++)
  {
	...
	$cantCitas = count($citas);
		...
		<tr style="background-color:<? echo $color;?>">
			...
			<td><div style="font-size:11px"><?php if($citas[$j]->Atencion==1){ echo "S. Medina"; } else { if($citas[$j]->Atencion==2){ echo "Dra. Fuentes"; }else { echo "Carol"; }};?></div></td>
